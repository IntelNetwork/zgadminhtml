    ...
    <div class="wrapper-content">
      <div class="box">
          ...
          <div class="table">
            ...
            <!-- 添加 -->
            <el-dialog title="添加类型" :visible.sync="dialogTableVisible">
...
<script>
import { type } from '../../../assets/js/industry'
export default {
  data() {
    return {
      tableData:[],
  ...
  methods: {
    //   分页查询
    getPermissionByRole(){
        let url =`${type.page}?pageNo=${this.current}&pageSize=${this.size}`;
        let data = {
            name: this.input
        }
    ...
    // 删除权限
    deleted(item){
        let urls = `${type.delect}?id=${item.id}`
    ...
    // 确定添加
    addResources(){
        this.axios.post(`${type.add}`,this.ruleForm1).then(res=>{
            // console.log(res.data)
            if(res.data.code==200){
