              ...
              <el-table
                ...
                <el-table-column
                    ...
                    <template  slot-scope="scope">
                        <span v-if="scope.row.taskState==0">未发布</span>
                        <span v-if="scope.row.taskState==1">待审核</span>
                        <span v-if="scope.row.taskState==2">已下架</span>
                        <span v-if="scope.row.taskState==3">发布（竞标中）</span>
                        <span v-if="scope.row.taskState==4">托管赏金</span>
                        <span v-if="scope.row.taskState==5">开始工作</span>
                        <span v-if="scope.row.taskState==6">提交验收</span>
                        <span v-if="scope.row.taskState==7">确认验收</span>
                        <span v-if="scope.row.taskState==8">支付赏金</span>
                        <span v-if="scope.row.taskState==9">-审核未通过</span>
                        <span v-if="scope.row.taskState==99">任务过期</span>
                    </template>
                </el-table-column>
                <el-table-column
                    prop="industryId"
...
<script>
import { task } from '../../../assets/js/industry'
export default {
  data() {
    return {
      tableData:[],
  ...
  methods: {
    //   分页查询
    getPermissionByRole(){
        let url =`${task.page}?pageNo=${this.current}&pageSize=${this.size}`;
        let data = {
            name: this.input
        }
    ...
    // 删除权限
    deleted(item){
        let urls = `${task.delect}?id=${item.id}`
    ...
    // 确定添加
    addResources(){
        this.axios.post(`${task.add}`,this.ruleForm1).then(res=>{
            // console.log(res.data)
            if(res.data.code==200){
