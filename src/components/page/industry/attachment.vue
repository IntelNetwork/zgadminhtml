      ...
      <div class="box">
          ...
          <div class="sel-search">
            <el-form ref="form" label-width="120px" class="demo-form-inline" :inline="true">
                ...
                <el-form-item>
                    <!-- <el-button  @click="dialogTableVisible = true" type="success" >添加</el-button> -->
          ...
          <div class="table">
            ...
            <!-- 添加 -->
            <el-dialog title="添加附件" :visible.sync="dialogTableVisible">
                <el-form :model="ruleForm1" status-icon  ref="ruleForm" label-width="140px" class="demo-ruleForm">
                    <el-form-item label="中文名" prop="cnName">
                        <el-input type="text" v-model="ruleForm1.cnName" autocomplete="off"></el-input>
                    </el-form-item>
                    <!-- <el-form-item label="任务类型编码" prop="taskId">
...
<script>
import { attachment } from '../../../assets/js/industry'
export default {
  data() {
    return {
      tableData:[],
  ...
  methods: {
    //   分页查询
    getPermissionByRole(){
        let url =`${attachment.page}?pageNo=${this.current}&pageSize=${this.size}`;
        let data = {
            name: this.input
        }
    ...
    // 删除权限
    deleted(item){
        let urls = `${attachment.delect}?id=${item.id}`
    ...
    // 确定添加
    addResources(){
        this.axios.post(`${attachment.add}`,this.ruleForm1).then(res=>{
            // console.log(res.data)
            if(res.data.code==200){
    ...
    removed(){
        ...
        parm = arrtemp.join(",");
        url = `${attachment.batchdelect}?ids=${parm}`;
