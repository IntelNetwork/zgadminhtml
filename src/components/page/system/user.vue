            ...
            <el-dialog title="编辑" :visible.sync="dialogTableVisible" width="50%">
                    ...
                    <el-form ref="form" label-width="110px" style="padding-top:30px;">
                        <el-form-item label="姓名" required>
                            <el-input v-model="usernames" :disabled="true"></el-input>
                        </el-form-item> 
                        <el-form-item label="真实姓名" required>
                            <el-input v-model="realname" placeholder="请输入内容"></el-input>
                        </el-form-item> 
                        <el-form-item label="电话" required>
                            <el-input v-model="phones" :disabled="true"></el-input>
                        </el-form-item> 
                        ...
                        <el-form-item label="权限选择" style="width:500px;">
                            <el-checkbox-group v-model="checkedCities" @change="handleCheckedCitiesChange">
                                <el-checkbox style="margin:0px 20px 15px 0;" border v-for="city in cities" :label="city.id" :key="city.id">{{city.roleName}}</el-checkbox>
                            </el-checkbox-group>
                        </el-form-item>  
                        <el-form-item>
...
export default {
  ...
  methods: {
    ...
    //编辑
    tableConfirm(){
         let length = this.userRole.length
         this.checkedCities.forEach((index)=>{
            let roleIds = {
                    roleId:index
            };
            this.userRole[length] = roleIds
        })
        let url = `${user.userEdit}`;
        let data = {
            // userDto :this.userDto,
            adminFlag: this.adminFlag,
            avatar :this.avatar,//头像
            ...
            realname:this.realname,
            status:this.status,
            userRoleDtos:this.userRole,
            username:this.usernames,//用户名
        }
